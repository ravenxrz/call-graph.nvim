...
vim.api.nvim_win_get_cursor = vim.api.nvim_win_get_cursor or function(win) return {1, 0} end
...
vim.api.nvim_create_buf = vim.api.nvim_create_buf or function() return 2 end
...
vim.api.nvim_open_win = vim.api.nvim_open_win or function() return 3 end
vim.api.nvim_win_is_valid = vim.api.nvim_win_is_valid or function() return false end
...
describe("Event callbacks", function()
  ...
  edge.sub_edges = {
    {
      ...
      end_row = 1,
      end_col = 10,
      to_string = function() return "SubEdge" end
    }
  }
  ...
  before_each(function()
    ...
    -- 模拟vim API函数
    vim.api.nvim_buf_is_valid = function(bufid) return true end
    ...
    vim.api.nvim_buf_line_count = function(bufid) return 0 end
  ...
  it("should jump to position when using gd on a node", function()
    ...
    -- 模拟被调用时的上下文
    vim.api.nvim_win_get_cursor = function() return {1, 5} end -- 第1行，第5列
  ...
  it("should jump to position when using gd on a different node", function()
    ...
    -- 模拟光标在第2行(index=1)
    vim.api.nvim_win_get_cursor = function() return {2, 5} end
  ...
  it("should show node info when using K on a node", function()
    ...
    vim.api.nvim_win_get_cursor = function() return {1, 5} end
