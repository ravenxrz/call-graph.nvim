...
local g_caller = nil -- Changed to a single variable
...
function Caller.generate_call_graph(opts, call_type)
  ...
  local function on_graph_generated(root_node, nodes, edges)
    last_buf_id = caller.view:draw(root_node, nodes, edges)
...
function Caller.get_caller(opts, call_type)
  if opts.reuse_buf and g_caller and last_call_type == call_type then -- Check g_caller directly
    return g_caller
  else
    local view
    ...
    g_caller = caller -- Assign directly
